...
export default function Patients({ session }) {
  ...
  async function fetchPatients() {
    ...
    const { data } = await supabase
      .from('patients').select('*').order('nom')
    setPatients(data || [])
    setLoading(false)
  }
      ...
      <div style={S.detail}>
        {!selected ? (
          ...
          <div style={S.detailInner}>
            ...
            {/* Notes */}
            {selected.notes && (
              <div style={{...S.pdCard, marginBottom:16}}>
...
const S = {
  ...
  listPanel:   { width:320, borderRight:'1px solid #DDE5EF', background:'#fff', display:'flex', flexDirection:'column', flexShrink:0 },
  listHeader:  { padding:'14px 16px 10px', borderBottom:'1px solid #DDE5EF' },
  search:      { width:'100%', padding:'8px 12px', border:'1.5px solid #DDE5EF', borderRadius:8, fontSize:13, fontFamily:'DM Sans, sans-serif', outline:'none' },
  ...
  modalBox:    { background:'#fff', borderRadius:16, width:'100%', maxWidth:580, boxShadow:'0 8px 32px rgba(12,68,124,0.18)', display:'flex', flexDirection:'column', maxHeight:'90vh' },
  ...
  frow3:       { display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 },
